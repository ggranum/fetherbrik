...
import static com.fetherbrik.core.json.DefaultObjectMapperProvider.mapper;
...
//        System.setProperty("DATE_TEST_VALUE", "${}");
      ...
      Map<String, ?> map = new JsonConfigurationReader(Env.DEVELOPMENT, mapper()).read(b.toString(), new HashMap<>());
    ...
    public void testReadFromMap() {
        ...
        try {
            File f = File.createTempFile("fetherbrik.dev", ".json");
            f.deleteOnExit();
            FileWriter w = new FileWriter(f);
            w.write(b.toString());
            w.close();
          Map<String, ?> map = new JsonConfigurationReader(Env.DEVELOPMENT, mapper()).read(f, new HashMap<>());
